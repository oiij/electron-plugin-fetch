...
export function electronFetchAdapter(config: InternalAxiosRequestConfig & { stream?: boolean, preload?: ElectronPluginFetchPreloadReturns }): AxiosPromise {
  return new Promise((resolve, reject) => {
    const controller = new AbortController()
    const { baseURL, url, method, params, paramsSerializer, data, headers, timeout, maxRedirects, signal, validateStatus, proxy, stream, preload } = config
